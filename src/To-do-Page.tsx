  ...
  return (
    <section>
      ...
      <p className="description">
        Utilizing a to-do list is an invaluable tool for organizing tasks and
        ensuring nothing is overlooked. Begin by entering tasks into the list,
        ensuring each is clearly defined and prioritized. Additionally, consider
        incorporating timers to help manage time effectively. This simple yet
        effective approach enhances productivity and helps individuals stay on
      ...
      <article className="setTask">
        <input
          ...
          placeholder={editing ? `Edit Task ${editIndex + 1}` : "Add Task"}
        />
        <button
          className="inputForms"
          onClick={() => (editing ? editTask() : addTask())}
        >
          {editing ? "Edit" : "Add"}
        </button>
      </article>
          ...
          {tasks.map((task, index) => (
            <li className={`listItems`} key={index}>
              ...
              <div>
                ...
                <button type="button" onClick={() => toggleEditMode(index)}>
